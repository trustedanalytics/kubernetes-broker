...
function find_other_nodes {
  for NUM in `seq 1 $NUM_NODES`; do
      NODE_SERVICE_HOST="${SERVICE_LABEL^^}_NODE${NUM}_SERVICE_HOST"
      NODE_SERVICE_PORT="${SERVICE_LABEL^^}_NODE${NUM}_SERVICE_PORT"

      # if set
      if [ -n "${!NODE_SERVICE_HOST}" ]; then
        # if not its own IP, then add it
        if [ $(expr "$HOSTNAME" : "${SERVICE_LABEL}-node${NUM}-.*") -eq 0 ]; then

          echo "Checking remote mysql node - ${!NODE_SERVICE_HOST}"
          # if not the first bootstrap node add comma
          if [ $WSREP_CLUSTER_ADDRESS != "gcomm://" ]; then
            WSREP_CLUSTER_ADDRESS="${WSREP_CLUSTER_ADDRESS},"
          fi

          #check mysql node health
          ERROR_MSG=`mysql -e "SHOW DATABASES;" --password=$MYSQL_ROOT_PASSWORD -h ${!NODE_SERVICE_HOST} --port ${!NODE_SERVICE_PORT} 2>/dev/null`
          if [ "$ERROR_MSG" != "" ]; then
            NODE_HOST_EXISTS="true"
            # append
...
if [ -n "$GALERA_CLUSTER" ]; then
  ...
  if [ -z "$WSREP_CLUSTER_ADDRESS" ] || [ "$WSREP_CLUSTER_ADDRESS"="gcomm://" ]; then
    ...
    # if SERVICE_LABEL = master then check others health, if not health start running as first node
  echo "Trying to find other nodes"
  find_other_nodes
